defmodule KvStore.MapReduce.Scheduler do
  use GenServer
  ...
  def init(:ok) do
    ...
    case Application.get_env(:kv_store, :persistence, false) do
  ...
  def terminate(_reason, {jobs, _refs, _seed}) do
    case Application.get_env(:kv_store, :persistence, false) do
      true  -> GenServer.call(KvStore.Persistence.Scanner, {:close, jobs}, 1000);
      false -> nil
    end
  end
  ...
  def handle_cast({:finished, job_id, result}, {jobs, refs, seed}) do
    ...
    ref = Enum.find(refs, fn({_key, value}) -> value == job_id end)
  ...
  def handle_call({:new_job, type, bucket}, _from, {jobs, refs, job_id}) do
    {:ok, pid} = KvStore.MapReduce.Job.start(job_id, type, bucket)
  ...
  def handle_call({:new_word_count_job, bucket, key_name}, _from, {jobs, refs, job_id}) do
    ...
    {response, updated_state} = case Enum.member?(keys, key_name) do
      ...
      true  ->
        {:ok, pid} = KvStore.MapReduce.WordCounter.start(self(), job_id, bucket, key_name)
  ...
  def handle_call({:get_job_result, job_id}, _from, {jobs, _refs, _seed} = state) do
    result = case Map.fetch(jobs, job_id) do
      :error        -> :no_such_job;
      {:ok, result} -> result
    end
